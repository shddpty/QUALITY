...
def loadMfDIMvMlDataFromMat(mat_path, fold_mat_path, fold_idx=0):
    ...
    labels, inc_view_indicator, inc_label_indicator = labels[sample_index], inc_view_indicator[sample_index], \
    inc_label_indicator[sample_index]
...
class IncDataset(Dataset):
    def __init__(self, mat_path, fold_mat_path, training_ratio=0.7, val_ratio=0.15, fold_idx=0, mode='train',
                 semisup=False):
        inc_mv_data, inc_labels, labels, inc_V_ind, inc_L_ind, total_sample_num = loadMfDIMvMlDataFromMat(mat_path,
                                                                                                          fold_mat_path,
                                                                                                          fold_idx)
...
class RecoverDataset(Dataset):
    def __init__(self, mv_data, labels, inc_V_ind, inc_L_ind):
        self.mv_data = mv_data
        self.labels = labels
        self.inc_V_ind = inc_V_ind
        self.inc_L_ind = inc_L_ind

    def __len__(self):
        return len(self.labels)

    def __getitem__(self, index):
        data = [v_data[index] for v_data in self.mv_data]
        label = self.labels[index]
        inc_V_ind = self.inc_V_ind[index]
        inc_L_ind = self.inc_L_ind[index]
        return data, label, inc_V_ind, inc_L_ind




def getComDataloader(matdata_path, training_ratio=0.7, val_ratio=0.15, mode='train', batch_size=1, num_workers=1,
                     shuffle=False):
...
def getIncDataloader(matdata_path, fold_matdata_path, training_ratio=0.7, val_ratio=0.15, fold_idx=0, mode='train',
                     batch_size=1, num_workers=1, shuffle=False):
    dataset = IncDataset(matdata_path, fold_matdata_path, training_ratio=training_ratio, val_ratio=val_ratio, mode=mode,
                         fold_idx=fold_idx)
    dataloder = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True)
    return dataloder, dataset
...
if __name__ == '__main__':
    ...
    dataloder, dataset = getIncDataloader('D:/0paper/LMVCAT-main/data/corel5k/corel5k_six_view.mat',
                                          'D:/0paper/LMVCAT-main/data/corel5k/corel5k_six_view_MaskRatios_0.5_LabelMaskRatio_0.5_TraindataRatio_0.7.mat',
                                          training_ratio=0.7, mode='train', batch_size=3, num_workers=2)
